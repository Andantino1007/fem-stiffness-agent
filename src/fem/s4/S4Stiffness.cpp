...
void validateInput(const S4ElementInput& input) {
...
LocalGeometry buildLocalGeometry(const S4ElementInput& input) {
...
S4StiffnessComponents computeLocalComponents(const S4ElementInput& input, const LocalGeometry& geometry) {
    S4StiffnessComponents components;
    ...
    // 横向剪切采用中心点项，并由完整积分残差提取仅作用于 uz 的边稳定化。
    const ShapeData centerShape = evaluateShape(geometry, 0.0, 0.0);
    BMatrix2 centerShearB{};
    for (std::size_t node = 0; node < kNodeCount; ++node) {
        const std::size_t base = node * kDofPerNode;
        centerShearB[0][base + 2] = centerShape.dNdx[node];
        centerShearB[0][base + 4] = centerShape.values[node];
        centerShearB[1][base + 2] = centerShape.dNdy[node];
        centerShearB[1][base + 3] = -centerShape.values[node];
    }
    addBtDB(components.shear, centerShearB, shearMaterial, 4.0 * centerShape.detJ);
    ...
    for (const auto& point : integrationPoints) {
        ...
        addBtDB(fullShear, shearB, shearMaterial, shape.detJ);
    }

    Matrix24 centerDisplacementShear;
    BMatrix2 centerDisplacementB{};
    for (std::size_t node = 0; node < kNodeCount; ++node) {
        const std::size_t uz = node * kDofPerNode + 2;
        centerDisplacementB[0][uz] = centerShape.dNdx[node];
        centerDisplacementB[1][uz] = centerShape.dNdy[node];
    }
    addBtDB(centerDisplacementShear, centerDisplacementB, shearMaterial, 4.0 * centerShape.detJ);
    ...
    double edgeResidual = 0.0;
    for (const auto& edge : edges) {
        const std::size_t first = edge[0] * kDofPerNode + 2;
        const std::size_t second = edge[1] * kDofPerNode + 2;
        edgeResidual -= fullShear(first, second) - centerDisplacementShear(first, second);
        edgeResidual -= fullShear(second, first) - centerDisplacementShear(second, first);
    }
    const double stabilization = (3.0 / 5.0) * edgeResidual / 8.0;
    if (!std::isfinite(stabilization) || stabilization < 0.0) {
        throw std::runtime_error("invalid transverse shear stabilization");
    }
    for (const auto& edge : edges) {
        const std::size_t first = edge[0] * kDofPerNode + 2;
        const std::size_t second = edge[1] * kDofPerNode + 2;
        components.shear(first, first) += stabilization;
        components.shear(second, second) += stabilization;
        components.shear(first, second) -= stabilization;
        components.shear(second, first) -= stabilization;
    }
...
S4StiffnessComponents computeS4StiffnessComponents(const S4ElementInput& input) {
    ...
    const S4StiffnessComponents local = computeLocalComponents(input, geometry);

    S4StiffnessComponents global;
...
Matrix24 computeS4Stiffness(const S4ElementInput& input) {
    return computeS4StiffnessComponents(input).total;
}
