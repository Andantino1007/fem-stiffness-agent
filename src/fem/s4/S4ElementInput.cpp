...
S4ElementInput readS4ElementInput(const std::filesystem::path& path) {
    ...
    result.referenceMatrixPath = matchString(text, "reference_matrix");
    result.implementationMatrixPath = matchString(text, "implementation_matrix");
    ...
    if (result.nodes.size() != 4) {
        throw std::runtime_error("S4 element input requires exactly 4 nodes");
    }
