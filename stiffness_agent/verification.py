...
DEFAULT_REPORT = ROOT / "docs" / "verification" / "样本001验证报告.md"

S4_SOURCES = [
    ROOT / "src" / "fem" / "s4" / "CsvMatrixIO.cpp",
    ROOT / "src" / "fem" / "s4" / "Matrix24.cpp",
    ROOT / "src" / "fem" / "s4" / "MatrixCompare.cpp",
    ROOT / "src" / "fem" / "s4" / "S4ElementInput.cpp",
    ROOT / "src" / "fem" / "s4" / "S4Stiffness.cpp",
]
...
def compile_verification_binaries() -> tuple[int, Path, Path]:
    ...
    cli_path = executable_path("s4_stiffness_cli")
    tests_path = executable_path("s4_stiffness_tests")
    compiler = compiler_command()
    flags = compiler_flags()
    source_args = [str(path.relative_to(ROOT)) for path in S4_SOURCES]

    compile_cli = [
        *compiler,
        *flags,
        *source_args,
        "src/cli/s4_stiffness_cli.cpp",
        "-o",
...
def sample_matrix_paths(meta_path: Path) -> tuple[Path, Path, dict[str, Any]]:
    ...
    reference = payload.get("reference_matrix")
    implementation = payload.get("implementation_matrix")
