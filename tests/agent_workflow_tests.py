...
--- a/src/fem/s4/S4Stiffness.cpp
+++ b/src/fem/s4/S4Stiffness.cpp
@@ -1,1 +1,1 @@
-old
+new
END_UNIFIED_DIFF"""
        diff = extract_unified_diff(response)
        self.assertEqual(validate_unified_diff(diff), ["src/fem/s4/S4Stiffness.cpp"])
...
"""
    ...
    def test_rejects_dangerous_added_call(self) -> None:
        diff = """--- a/src/fem/s4/S4Stiffness.cpp
+++ b/src/fem/s4/S4Stiffness.cpp
@@ -1,1 +1,1 @@
-old
+system(\"unsafe\");
...
*** End Patch"""
    ...
    def test_semantic_patch_fingerprint_ignores_comments_and_hunk_lines(self) -> None:
        first = """--- a/src/fem/s4/S4Stiffness.cpp
+++ b/src/fem/s4/S4Stiffness.cpp
@@ -10,3 +10,3 @@
-// 原注释
+// 新注释
...
+value = shape[node];
"""
        second = """--- a/src/fem/s4/S4Stiffness.cpp
+++ b/src/fem/s4/S4Stiffness.cpp
@@ -20,5 +20,5 @@
-// 另一种旧注释
+// 另一种新注释
...
"""
    ...
    def test_updates_dedicated_progress_document(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            progress_path = Path(directory) / "项目进度与下一步.md"
