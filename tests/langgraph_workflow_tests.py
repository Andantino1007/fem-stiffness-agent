...
from model_api_client import OpenAIClientError, OpenAIHttpError
from langgraph_workflow import (
    build_graph,
    build_planned_iteration,
...
class GraphWorkflowTests(unittest.TestCase):
    def test_retries_connection_and_temporary_http_errors(self) -> None:
        ...
        self.assertTrue(is_retryable_agent_error(ValueError("invalid agent payload")))
    ...
    def test_replanning_clears_stale_duplicate_failure(self) -> None:
        state = {
            "iteration_number": 20,
            "current_error": 0.041,
            "iteration": {
                "failure": "stale duplicate failure",
                "duplicate_patch": True,
            },
        }
        iteration = build_planned_iteration(state, {"hypothesis_id": "fresh"}, 2)
        self.assertEqual(iteration["planning_attempt"], 2)
        self.assertNotIn("failure", iteration)
        self.assertNotIn("duplicate_patch", iteration)
