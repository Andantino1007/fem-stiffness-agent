...
本目录保存在线 API 客户端、多 Agent 编排和 Abaqus 数据工具。
...
| `graph.py` | 默认 LangGraph 编排、checkpoint、重试和恢复 | 是 |
| `agents.py` | Agent 实现、提示词组装和公共工具 | 是 |
| `agent_openai_client.py` | OpenAI 兼容 API 客户端 | 是 |
...
默认引擎为 `langgraph`。API 节点失败时退出码为 `3`，终端会打印恢复命令：
...
LangGraph `run` 会自动先执行同样的 API Preflight 节点。
...
正式自动迭代只允许 Developer 修改 `src/shell/ShellStiffness.cpp`。补丁先经过路径和危险调用校验，再经过 `git apply --check --recount`、C++/Catch2、真实 Abaqus 误差比较和 Reviewer 决策。未改善时自动恢复。

退出码 `0` 表示至少接受一轮改进或已经达到目标；退出码 `2` 表示本批次没有改进被接受，候选代码已经回滚。

启动时会读取并补全 `workflow/experiment-memory.json`。每轮结束后记录理论摘要、补丁摘要、误差变化、失败机理、`do_not_repeat` 和下一步焦点，并在后续 run 中注入在线 Agent。

`OPENAI_API_STYLE=auto` 是在线 API 接口格式回退，不是离线模式。
