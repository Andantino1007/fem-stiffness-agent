...
from model_api_client import OpenAIClientError, OpenAIHttpError, load_openai_config
...
CHECKPOINT_PATH = ROOT / "build" / "workflow" / "checkpoints" / "checkpoint.sqlite"
...
def is_retryable_agent_error(exc: Exception) -> bool:
    return is_retryable_api_error(exc) or isinstance(
...
def build_planned_iteration(
    state: GraphState, plan: dict[str, Any], attempt: int
) -> dict[str, Any]:
    return {
...
def planner_node(state: GraphState) -> dict[str, Any]:
    ...
    iteration = build_planned_iteration(state, plan, attempt)
    return {
        "experiment_plan": plan,
        "iteration": iteration,
        "planning_attempt": attempt,
...
def build_graph(checkpointer: SqliteSaver):
    ...
    graph.add_node("planner", planner_node, retry_policy=AGENT_RETRY_POLICY)
    graph.add_node("theory", theory_node, retry_policy=AGENT_RETRY_POLICY)
    graph.add_node("developer", developer_node, retry_policy=AGENT_RETRY_POLICY)
    ...
    graph.add_node("reviewer", reviewer_node, retry_policy=AGENT_RETRY_POLICY)
...
def run_graph(
    ...
    try:
        ...
        console(f"恢复命令：python -m stiffness_agent resume {run_id}")
        return 3
