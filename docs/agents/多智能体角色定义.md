...
本文档定义本项目的多 Agent 协作口径。当前阶段已经支持 OpenAI API 真实 Agent：Theory Research Agent、Developer Agent、Reviewer Agent 可以调用 OpenAI API；Abaqus Data Agent 和 Test Agent 保持确定性工具节点。
...
输出：
- `build/workflow/runs/<run_id>/state.json`
- 本轮流程日志。
...
阶段 2 当前输出：
- 检查 `sample_001_reference.csv` 是否为 24 x 24。
- 标记当前是否为真实 Abaqus 矩阵。
...
阶段 2 当前输出：
- 复用 `python -m stiffness_agent verify` 中的 Catch2 测试。
- 提取是否通过。
