...
| train | `s4_train_003_node_down` | `data/abaqus/raw/s4_train_003_node_down_STIF1.mtx` | 376 | 完成，0 errors，1 条畸变输入警告 |
| validation | `s4_validation_001_skew` | `data/abaqus/raw/s4_validation_001_skew_STIF1.mtx` | 380 | 完成，0 errors，0 warnings |

四个 CSV 均通过 `24 x 24`、有限值和对称性校验。畸变警告与两个训练算例的覆盖目标一致，分析均正常完成。
...
| train | `s4_train_003_node_down` | 0.443257 | 2.02722e+09 | 8.33579e-17 |
| validation | `s4_validation_001_skew` | 0.209217 | 2.79829e+08 | 3.01256e-17 |

`verify-dataset` 对包含原始 `sample_001` 的完整训练集给出：4 个训练样本，平均 Frobenius 相对误差 `0.240023925`，最差样本为 `s4_train_003_node_down`，误差 `0.443257`。验证集已就绪，最差误差 `0.209217`。测试集仍为空，因此 `final_evaluation_ready=false`；本报告不宣称达到 `1%` 数值验收目标。

## 测试结果

- `python -m shell_agent verify-dataset`：通过；训练 4 个、验证 1 个，C++ 适配器测试通过。
- Python 工作流：`python -m unittest discover -s tests -p '*_tests.py' -v`，34 个测试通过。
- C++ 数值测试：Intel oneAPI 2025.1 `icx`，6 个测试用例、43 个断言全部通过。
- 转换器回归：完整对称矩阵不重复累加、三角矩阵自动补齐、不一致对称条目拒绝，3 个测试全部通过。

完整 Abaqus 作业证据保存在 `data/abaqus/evidence/`，每份日志均记录 `MATRIX OUTPUT FORMAT - COORDINATE`、分析完成和错误计数。
