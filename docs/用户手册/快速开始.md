...
`data/implementation/*.csv` 实现矩阵显示为已修改。验证后先运行
`git diff -- data/implementation`，确认差异只来自本次生成的实现矩阵；不要把
这类生成差异作为文档修改提交，也不要改动
`data/reference/abaqus/matrices/` 中的参考矩阵。具体处理见
[项目可运行检查](项目可运行检查.md#macosclang-验证后实现矩阵显示为已修改)。
