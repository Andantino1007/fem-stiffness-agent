...
ALLOWED_METHODS = {"matrix_generate", "reaction_odb"}
REQUIRED_FORBIDDEN_CHANGES = {
    "不得使用C++结果作为参考矩阵",
    "不得覆盖已有样本",
    "不得生成没有Abaqus来源记录的CSV",
}
...
def _validate_nodes(nodes: Any, sample_id: str) -> None:
    if not isinstance(nodes, list) or len(nodes) != 4:
        raise ValueError(f"样本 {sample_id} 必须包含4个节点")
    ...
    second = tuple(coordinates[3][i] - coordinates[0][i] for i in range(3))
    cross = (
    ...
    if math.sqrt(sum(value * value for value in cross)) <= 1.0e-12:
        raise ValueError(f"样本 {sample_id} 四边形面积退化")
...
def validate_dataset_plan(
    ...
    if plan.get("schema_version") != 1 or missing:
    ...
    if plan["generation_method"] not in ALLOWED_METHODS:
    ...
    existing_ids = _existing_sample_ids(dataset_path)
    ...
    for sample in samples:
        ...
        _validate_nodes(sample.get("nodes"), identifier)
